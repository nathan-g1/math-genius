...
public class MathGeniusClient {
    ...
    public static void main(String[] args) {
        ...
        // Create and start multiple client instances
        for (int i = 0; i < numClients; i++) {
            Thread clientThread = new Thread(new MathGeniusClientRunner());
            clientThread.start();
        }
    }

    private static class MathGeniusClientRunner implements Runnable {
        @Override
        public void run() {
            ...
            for (int i = 0; i < totalRequests; i++) {
                ...
                switch (operation) {
                    case 0:
                        double resultAdd = stub.magicAdd(MagicAddRequest.newBuilder().setInput1(param1)
                                .setInput2(param2).build()).getSum();
